...
// import { loginUserInputFactory, registerUserInputFactory, userFactory } from '../../factories/user.factory';
...
//       const user = await e2e.dbTestUtils.saveOne(userFactory.buildOne(userLoginInput));
...
//       const result = await request(e2e.app.getHttpServer()).post(GQL).send(gqlReq).expect(200);
...
//   describe.each(Object.values(SocialProviderTypes))(`SocialAuth %s`, (provider: SocialProviderTypes) => {
...
//       jest.spyOn(Strategy, 'userProfile').mockImplementation((token: any, func: any) => {
